-->-->-- src/Frapid.Web/Areas/Frapid.WebsiteBuilder/db/SQL Server/1.x/1.0/src/99.sample-data/website.sample.sql --<--<--
DELETE FROM website.contents;
DELETE FROM website.menu_items;
DELETE FROM website.menus;
DELETE FROM website.categories;
...
SELECT 'Welcome to Frapid', 'frapid,cms', 'welcome-to-frapid', website.get_category_id_by_category_alias('default'), getutcdate(), 0, 'Homepage of Frapid Framework', 1, '<div class="ui basic inverted attached segment" id="banner">
...
SELECT 'Terms of Use', '', 'terms-of-use', website.get_category_id_by_category_alias('legal'), getutcdate(), 0, 'Terms of Use', 0, '# Terms of Use

This document is empty.', '<h1 id="terms-of-use">Terms of Use</h1>
<p>This document is empty.</p>';

INSERT INTO website.contents(title, tags, alias, category_id, publish_on, is_draft, seo_description, is_homepage, markdown, contents)
SELECT 'Privacy Policy', '', 'privacy-policy', website.get_category_id_by_category_alias('legal'), getutcdate(), 0, 'Privacy Policy', 0, '# Privacy Policy
...
SELECT 'Corporate Headquarters', 'Your Office Name', '', 'Address', 'City', 'State', 'Country', '000', '000 000 000', '', 'info@frapid.com', 0, 1, 1 UNION ALL
SELECT 'United States', 'John Doe', 'Client Partner', '', '', '', '', '', '', 'Texas', 'info@frapid.com', 0, 1, 1;
