# finance.get_cash_flow_statement function:

```plpgsql
CREATE OR REPLACE FUNCTION finance.get_cash_flow_statement(_date_from date, _date_to date, _user_id integer, _office_id integer, _factor integer)
RETURNS json
```
